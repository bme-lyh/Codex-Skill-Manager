...
Codex 辅助复核是可选功能，默认关闭。启用前需要：

1. 安装并登录 **Codex CLI**；
2. 在应用的“设置”中启用 Codex 辅助复核；
3. 确认你的 Codex 账户有可用额度。

复核会消耗 Codex 额度，所需时间与 Skill 数量、文件大小、所选模型和推理强度有关。应用会按分组显示当前进度，并为每个 Skill 分别给出简短结论。不开启此功能时，本地风险检查和其他管理功能仍可正常使用。
...
当前版本为 **0.7.4**，主要面向 Windows 10/11。项目仍在完善，重要操作前请查看计划并保留自动备份。
