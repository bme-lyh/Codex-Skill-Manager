...
Codex Skill Manager 是面向 Windows 的本地 Codex Skills 管理工具。它帮助你检查、
安装、更新、分组和恢复 Skills，并让每次写入都有明确目标、事务记录和恢复入口。

[下载 v0.10.0](https://github.com/bme-lyh/Codex-Skill-Manager/releases/tag/v0.10.0) ·
...
[![Codex Skill Manager 界面预览](docs/images/ui-carousel.gif)](docs/images/ui-carousel.gif)
...
| 任务 | 应用会做什么 |
|---|---|
| 安装 | 读取 GitHub 公共/私有仓库或本地目录，只安装明确选中的 Skills |
| 评估 | 阅读项目说明、发现 Skills、扫描真实目标，并区分必选、按条件触发和可选检查 |
...
普通 Skill 仓库直接使用标准安装，只复制选中的 Skill 目录。复杂项目可以额外运行
**Codex 增强分析**，查看项目说明并生成受支持的 Python 工具或 MCP 计划。增强分析
需要已安装并登录的 Codex CLI，会消耗 Codex 额度，但不会替代必选的本地评估。

发现 Codex Skill 后，选中的 Skill 默认安装到
`%USERPROFILE%\.codex\skills`；`.system` 始终只读。没有 Codex Skill 的普通项目
不会被静默写入全局 Skills 目录。不受支持的内容会保留为人工步骤；MCP 如需项目目录，
必须由使用者明确选择。
...
将结果与 `CodexSkillManager-0.10.0-SHA256SUMS.txt` 中的对应行比较。
...
- 不执行仓库脚本或自由形式命令。
- High 风险必须逐簇确认并填写原因；Critical 风险不可忽略。
- 报告级批量忽略只处理已知的 Medium 及以下风险。
- 替换前备份，卸载后进入隔离区；目标被外部修改后，回滚不会强行覆盖。
- Codex 增强分析默认只读；生成计划、批准权限和执行是分开的操作。

“未发现风险”不等于绝对安全。批准陌生来源前，请阅读[安全策略](SECURITY.md)。

## 从源码构建

需要 Go、Node.js、pnpm、WebView2 和 Wails v2：
...
Codex Skill Manager 0.10.0 支持 Windows 10/11，使用 [MIT License](LICENSE)。
