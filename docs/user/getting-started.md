...
1. 选择“Codex 一键安装”并扫描项目；
2. 应用按“本地结果→文件摘要→重点文件分析”生成项目概述、安全结论和安装方式；
3. 核对扫描结论；只有同意继续后，Codex 才会生成结构化安装计划；
4. 核对计划，选择 Skills，并逐项批准必要权限；
5. 如果计划包含 MCP，选择它要服务的真实 Git 或 SVN 项目目录；
6. 执行后在时间线中查看当前步骤、进度、错误和恢复建议。

应用只执行经过本地验证的类型化步骤：安装 Skills、从官方 PyPI 安装经过校验的
Python 工具、配置由应用管理的 Codex MCP。它不会执行仓库脚本或 Codex 自由生成
的命令；不能安全自动化的步骤会明确标为“手动”。来源上下文会按固定预算分层处理，
敏感凭据类文件只发送元数据；私有仓库使用者仍应在启用前确认自己的数据与额度要求。

生成计划并识别到 Python 工具时，应用会从官方 PyPI 把完整 Wheel 依赖下载到隔离暂存区，用来
