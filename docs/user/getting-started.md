...
链接。界面统一为“来源 → 评估 → 复核 → 执行”四步，不需要先选择复杂模式。
两种来源都会先：

1. 将 GitHub 版本解析为不可变 Commit，或确认明确的本地目录；
2. 将 GitHub 内容下载到隔离暂存目录，或把本地目录复制为有界、只供本次检查的快照；
3. 阅读项目说明和安装线索，分类项目并发现 Codex Skills；
4. 对实际 Skill 目标执行本地安全扫描；
5. 按“必选 / 按条件触发 / 可选”展示检查结果、证据和目标；
6. 只有评估为“可继续”或“需关注”时，才允许选择目标并执行。
...
适合还需要 Python 工具、Codex MCP 等集成的复杂仓库。它不会替代必选本地评估；
点击“运行增强项目扫描”才表示本次明确启用。使用前请在“设置”中检查 Codex CLI，
并确保它已安装、已登录且有可用额度。
